...
export default async function DallasAdmin() {
        ...
        <div className="main">
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 16, flexWrap: "wrap" }}>
            <div className="card" style={{ flex: 1, minWidth: 300, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <b>Data sources · live</b><span className="chip">{data.feeds.length} active</span>
              </div>
              <div style={{ marginTop: 12, fontFamily: "Menlo,monospace", fontSize: 10.5, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "var(--faint)" }}>DPD daily incident record</span>
                  <span style={{ color: "var(--faint)" }}>PENDING · confirmation layer</span>
                </div>
              </div>
              <div style={{ marginTop: 12, fontSize: 10.5, color: "var(--mut)" }}>
                Dallas dispatch is preliminary (REPORTED). Each source has an expected freshness
                window; outside it → flagged and excluded from status calc.
              </div>
            </div>
            <div className="card" style={{ flex: 1, minWidth: 260, padding: 16 }}>
              ...
              <div style={{ marginTop: 12, fontFamily: "Menlo,monospace", fontSize: 10.5, display: "flex", flexDirection: "column", gap: 9 }}>
                {[
                  ...
                  ["E-1 weather trigger", "NWS watch ∩ campus"],
                  ["Dispatch tier", "REPORTED · MONITOR/ELEVATED only"],
                  ["A-2 confirmed shooting", "0.25 mi · daily record"],
                  ["De-escalation", "ring clear + window expiry"],
                ].map(([k, v]) => (
